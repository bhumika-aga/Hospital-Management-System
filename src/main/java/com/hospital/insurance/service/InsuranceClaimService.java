...
public class InsuranceClaimService {
    ...
    public InsurerDTO getInsurerByPackageName(String packageName) {
        ...
        Insurer insurer = insurers.stream().filter(Insurer::getActive).findFirst()
    ...
    public ClaimInitiationResponse initiateClaim(ClaimInitiationRequest request) {
        ...
        ClaimRequest claimRequest = new ClaimRequest(request.getPatientName(), request.getAilment(),
                request.getTreatmentPackageName(), request.getTreatmentCost(), insurer.getInsurerName(),
                insurer.getPackageName(), insurer.getInsuranceAmountLimit(), balanceAmount);
        ...
        // Build response
        return new ClaimInitiationResponse(savedClaim.getId(), claimReferenceNumber, request.getPatientName(),
                insurer.getInsurerName(), request.getTreatmentCost(), insuranceCoverage, balanceAmount, "INITIATED",
                savedClaim.getClaimInitiatedDate(), expectedDisbursementDate,
                "Insurance claim initiated successfully. Patient needs to pay balance amount: ₹"
    ...
    private InsurerDTO convertToDTO(Insurer insurer) {
        return new InsurerDTO(insurer.getId(), insurer.getInsurerName(), insurer.getPackageName(),
                insurer.getInsuranceAmountLimit(), insurer.getDisbursementDurationDays(), insurer.getContactEmail(),
                insurer.getContactPhone(), insurer.getAddress(), insurer.getWebsite(), insurer.getActive());
    }

    private String generateClaimReferenceNumber(String insurerName) {
        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));
        String insurerCode = insurerName.replaceAll("\\s+", "").substring(0, Math.min(3, insurerName.length()))
                .toUpperCase();
