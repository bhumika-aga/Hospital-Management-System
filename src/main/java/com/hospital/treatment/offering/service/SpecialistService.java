...
public class SpecialistService {
    ...
    public List<SpecialistDTO> getSpecialistsByExpertise(String expertise) {
        return specialistRepository.findBySpecialization(expertise).stream().map(this::convertToDTO)
                .collect(Collectors.toList());
    }

    public List<SpecialistDTO> getSpecialistsByLevel(String level) {
        return specialistRepository.findByLevel(level.toUpperCase()).stream().map(this::convertToDTO)
                .collect(Collectors.toList());
    }

    public List<SpecialistDTO> getAvailableSpecialists(String specialization, String level) {
        return specialistRepository.findBySpecializationAndLevel(specialization, level.toUpperCase()).stream()
                .filter(Specialist::getAvailable).map(this::convertToDTO).collect(Collectors.toList());
    }
    ...
    private SpecialistDTO convertToDTO(Specialist specialist) {
        return new SpecialistDTO(specialist.getId(), specialist.getName(), specialist.getSpecialization(),
                specialist.getLevel(), specialist.getQualification(), specialist.getExperience(),
                specialist.getContactNumber(), specialist.getEmail(), specialist.getAvailable());
    }

    private Specialist convertToEntity(SpecialistDTO dto) {
        return new Specialist(dto.getName(), dto.getSpecialization(), dto.getLevel(), dto.getQualification(),
                dto.getExperience(), dto.getContactNumber(), dto.getEmail());
    }
}